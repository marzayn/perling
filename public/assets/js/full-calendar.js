            ...
            if (c == "'") {
                ...
                for (i2 = i + 1; i2 < len; i2++) {
                    if (format.charAt(i2) == "]") {
                        ...
                        if (
                            subres != formatDate(otherDate, subformat, options)
                        ) {
                            res += subres;
                        }
                        i = i2;
...
*/
        ...
        function setHeight(height) {
            ...
            bodyFirstCells.each(function (i, _cell) {
                if (i < rowCnt) {
                    ...
                    cell.find("> div").css(
                        "min-height",
                        (i == rowCnt - 1 ? rowHeightLast : rowHeight) -
                            vsides(cell)
                    );
                }
            });
	...
	-----------------------------------------------------------------------*/
        ...
        function buildDayTable() {
            ...
            dayBodyCellContentInners = dayBodyCells.find(
                ".fc-day-content > div"
            );
	...
	-----------------------------------------------------------------------*/
        ...
        function setHeight(height) {
            ...
            dayBodyFirstCellStretcher.height(
                bodyHeight - vsides(dayBodyFirstCell)
            );
        ...
        function setWidth(width) {
            ...
            if (allDayTable) {
                axisFirstCells = axisFirstCells.add(
                    allDayTable.find("th:first")
                );
            }
        ...
        function slotClick(ev) {
            if (!opt("selectable")) {
                ...
                var col = Math.min(
                    colCnt - 1,
                    Math.floor(
                        (ev.pageX - dayTable.offset().left - axisWidth) /
                            colWidth
                    )
                );
	...
	-----------------------------------------------------------------------------*/
        ...
        coordinateGrid = new CoordinateGrid(function (rows, cols) {
            ...
            var slotScrollerBottom =
                slotScrollerTop + slotScroller.outerHeight();
            function constrain(n) {
                return Math.max(
                    slotScrollerTop,
                    Math.min(slotScrollerBottom, n)
                );
            }
        ...
        function timePosition(day, time) {
            ...
            return Math.max(
                ...
                Math.round(
                    slotTop -
                        1 +
                        slotHeight * ((minutes % slotMinutes) / slotMinutes)
                )
            );
        }
	...
	-----------------------------------------------------------------------------------*/
        ...
        function draggableSlotEvent(event, eventElement, timeElement) {
            ...
            function updateTimeText(minuteDelta) {
                ...
                timeElement.text(
                    formatDates(newStart, newEnd, opt("timeFormat"))
                );
            }
        }
	...
	---------------------------------------------------------------------------------*/
        ...
        function elongateEvents(events, dayDelta, minuteDelta) {
            ...
            for (var e, len = events.length, i = 0; i < len; i++) {
                e = events[i];
                e.end = addMinutes(
                    addDays(eventEnd(e), dayDelta, true),
                    minuteDelta
                );
                normalizeEvent(e, options);
            }
        ...
        function skipHiddenDays(date, inc, isExclusive) {
            ...
            while (
                isHiddenDayHash[
                    (date.getDay() + (isExclusive ? inc : 0) + 7) % 7
                ]
            ) {
                addDays(date, inc);
            }
        ...
        function rangeToSegments(startDate, endDate) {
            ...
            var rangeCellOffsetFirst =
                dayOffsetToCellOffset(rangeDayOffsetStart);
            var rangeCellOffsetLast =
                dayOffsetToCellOffset(rangeDayOffsetEnd) - 1;
            ...
            for (var row = 0; row < rowCnt; row++) {
                ...
                if (segmentCellOffsetFirst <= segmentCellOffsetLast) {
                    // translate to cells
                    var segmentCellFirst = cellOffsetToCell(
                        segmentCellOffsetFirst
                    );
                    var segmentCellLast = cellOffsetToCell(
        ...
        function calculateVerticals(segments) {
            ...
            for (var rowI = 0; rowI < rowCnt; rowI++) {
                ...
                for (
                    ...
                    for (
                        var colI = segment.leftCol;
                        colI <= segment.rightCol;
                        colI++
                    ) {
